...
K3S_OIDC_RUNTIME="${K3S_OIDC_RUNTIME:-lima}"
...
preview_summary="would exercise the ${K3S_OIDC_RUNTIME_LABEL} k3s OIDC restart path and verify recovery"

emit_preview_result() {
  if json_mode; then
    jq -n \
      --arg force_mode "${OIDC_RECOVERY_FORCE_MODE}" \
      --arg configure_script "${K3S_OIDC_CONFIGURE_SCRIPT}" \
...
emit_result() {
  if json_mode; then
    ...
    jq -n \
      ...
      --arg forced_name "k3s" \
      --arg forced_node "${K3S_OIDC_NODE_NAME}" \
...
oidc_issuer_reachable_from_vm_once() {
  set +e
  k3s_exec "${K3S_OIDC_NODE_NAME}" sudo curl -fsS --max-time 5 --cacert "${MKCERT_CA_DEST}" \
...
run_configure_step() {
  ...
  record_step "configure" "running" "delegating to ${K3S_OIDC_CONFIGURE_SCRIPT}"

  set +e
  configure_output="$("${K3S_OIDC_CONFIGURE_SCRIPT}" --execute 2>&1)"
  CONFIGURE_EXIT_CODE=$?
  set -e
...
force_restart() {
  ...
  record_step "force" "running" "restarting k3s on ${K3S_OIDC_NODE_NAME} to force the ${K3S_OIDC_RUNTIME_LABEL} OIDC recovery path"

  set +e
  restart_output="$(k3s_exec "${K3S_OIDC_NODE_NAME}" sudo systemctl restart k3s 2>&1)"
  ...
  FORCED=true
  ok "restarted k3s on ${K3S_OIDC_NODE_NAME}"
...
run_recovery_checks() {
  ...
  if ! wait_for_oidc_issuer_from_vm "${OIDC_RECOVERY_ISSUER_WAIT_SECONDS}"; then
    SUMMARY="OIDC issuer ${OIDC_ISSUER_URL} was not reachable from ${K3S_OIDC_NODE_NAME} within ${OIDC_RECOVERY_ISSUER_WAIT_SECONDS}s after the forced k3s restart"
    record_step "recovery" "failed" "${SUMMARY}"
    POST_STATE="$(current_state)"
    emit_result
    exit 1
...
record_step "preflight" "${PRE_STATE}" "detected ${PRE_STATE} ${K3S_OIDC_RUNTIME_LABEL} OIDC runtime dependencies before the forced restart"
...
if [[ "${POST_STATE}" != "healthy" ]]; then
  ...
  SUMMARY="forced k3s restart completed but the ${K3S_OIDC_RUNTIME_LABEL} OIDC runtime dependencies are still degraded"
  record_step "postflight" "failed" "${SUMMARY}"
  emit_result
  exit 1
fi

record_step "postflight" "healthy" "${K3S_OIDC_RUNTIME_LABEL} OIDC runtime dependencies are healthy after the forced restart drill"
STATUS_CODE="forced_k3s_restart_recovered"
STATUS_GROUP="success"
RESULT_OK=true
SUMMARY="controlled ${K3S_OIDC_RUNTIME_LABEL} OIDC restart drill completed successfully"
emit_result
