...
@test "fails loudly with unknown owner wording when lsof is unavailable" {
  ...
  [ "${status}" -eq 1 ]
  [[ "${output}" == *"FAIL competing hypervisor VM detected: com.apple.Virtualization.VirtualMachine owner=unknown VM owner pid=301 RSS=3.0GiB; threshold is 2GiB (2 via KIND_PREFLIGHT_COMPETING_VM_RSS_GB); remediation: stop the owning VM application (inspect with: lsof -p 301)"* ]]
}
