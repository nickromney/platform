...
- `API_PROXY_ENABLED` (optional): set to `"true"` to force the SPA to call relative `/api/*` paths (pairs with `PROXY_API_URL`). Defaults to `false`.
...
```bash
# Build Docker image
docker build -t subnetcalc-react .

# Push to Azure Container Registry
az acr build --registry myacr --image subnetcalc-react:latest .
...
az webapp create \
  ...
  --name app-react \
  --plan asp-linux \
  --deployment-container-image-name myacr.azurecr.io/subnetcalc-react:latest
```
...
```bash
docker build -t subnetcalc-react .
docker run -p 8080:8080 subnetcalc-react
```
