...
@test "slicer build-workload-images calls the shared workload image builder directly" {
  run make -n -C "${REPO_ROOT}/kubernetes/slicer" build-workload-images
  ...
  [[ "${output}" == *'TAG="latest"'* ]]
  [[ "${output}" == *'kubernetes/scripts/build-local-workload-images.sh" --execute'* ]]
  [[ "${output}" != *'kubernetes/slicer/scripts/build-local-workload-images.sh'* ]]
}
