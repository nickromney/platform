...
allowed_python_execution=(
  "docs/tutorials/apim-get-started/tutorial01.sh"
...
if [[ "${#unexpected_python[@]}" -gt 0 ]]; then
  ...
  printf '  %s\n' "${unexpected_python[@]}" >&2
  printf 'Allowlist only approved shell wrappers that intentionally execute Python.\n' >&2
  exit 1
fi
...
echo "OK   shell audit (${count} script(s) scanned; ${entrypoint_count} executable entrypoint(s) validated; Python execution limited to approved shell wrappers)"
