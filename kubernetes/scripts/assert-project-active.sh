...
[ -n "${EXPECTED_PROJECT_PATH}" ] || fail "EXPECTED_PROJECT_PATH must be set"
command -v jq >/dev/null 2>&1 || fail "jq not found in PATH"
command -v kubectl >/dev/null 2>&1 || fail "kubectl not found in PATH"
[ -x "${PLATFORM_STATUS_SCRIPT}" ] || fail "Platform status helper not found or not executable: ${PLATFORM_STATUS_SCRIPT}"
[ -n "${kubeconfig_path}" ] || fail "KUBECONFIG_PATH or KUBECONFIG must be set"

status_json="$("${PLATFORM_STATUS_SCRIPT}" --execute --output json)" || fail "Failed to inspect local runtime status with ${PLATFORM_STATUS_SCRIPT}"
...
[ -n "${expected_project_key}" ] || fail "Unknown EXPECTED_PROJECT_PATH: ${EXPECTED_PROJECT_PATH}"
...
if [ "${overall_state}" = "conflict" ]; then
  fail $'Multiple tracked platform surfaces are active on this machine.\nInspect the full local runtime state with:\n  make status'
fi
...
case "${expected_project_state}" in
  absent|stopped)
    fail "${EXPECTED_PROJECT_PATH} is not running on this machine.
Inspect the local runtime state with:
  make status"
    ;;
  paused)
    fail "${EXPECTED_PROJECT_PATH} is paused on this machine.
Inspect the local runtime state with:
  make status"
    ;;
esac
...
if ! KUBECONFIG="${kubeconfig_path}" kubectl "${kubectl_args[@]}" >/dev/null 2>&1; then
  ...
  fail "${EXPECTED_PROJECT_PATH} is not reachable via kubeconfig ${kubeconfig_path}${context_note}.
Inspect the local runtime state with:
  make status"
fi
