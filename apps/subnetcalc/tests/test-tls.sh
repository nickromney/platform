...
# Verifies Mozilla Modern profile compliance on the subnetcalc compose TLS stack.
#
# Usage:
...
TLS cipher, protocol, and transport security tests for the subnetcalc compose TLS stack.
...
shell_cli_maybe_execute_or_preview_summary \
  usage \
  "would run subnetcalc TLS tests against ${TARGET} using ${CA_CERT} and redirect port ${HTTP_REDIRECT_PORT}"
