...
get_status() {
  local name="$1"
  limactl_no_agent list 2>/dev/null | awk -v n="$name" '$1==n {print $2}' | head -1
}
...
for i in $(seq 1 "$DESIRED_NODES"); do
  ...
  if [ -z "$status" ]; then
    ...
    limactl_no_agent start --name "$name" "$LIMA_CONFIG" --containerd none --tty=false --timeout=15m
  elif [ "$status" = "Stopped" ]; then
    echo "Starting existing Lima VM: ${name}"
    limactl_no_agent start "$name" --containerd none --tty=false --timeout=10m
  else
...
for _ in $(seq 1 60); do
  ...
  if [ "$running" -ge "$DESIRED_NODES" ]; then
    ...
    limactl_no_agent list | grep -E "^(NAME|${LIMA_INSTANCE_PREFIX})" || true
    exit 0
  fi
  sleep 5
done
