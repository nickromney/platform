...
@test "slicer sync-image-cache calls the shared image sync adapter directly" {
  run make -n -C "${REPO_ROOT}/kubernetes/slicer" sync-image-cache STAGE=900

  [ "${status}" -eq 0 ]
  [[ "${output}" == *'VARIANT_LABEL="Slicer"'* ]]
  [[ "${output}" == *'IMAGE_LIST_FILE="'*"kubernetes/slicer/preload-images.txt"* ]]
  [[ "${output}" == *'kubernetes/scripts/sync-local-image-cache.sh" --execute'* ]]
  [[ "${output}" != *'kubernetes/slicer/scripts/sync-local-image-cache.sh'* ]]
}
