...
- **Published language:** session cookies, `/oauth2/userinfo`, standard OIDC
  endpoints.
- **Identity products underneath:** Dex in-cluster, Keycloak in compose. Both
  are supporting detail behind the `oauth2-proxy` surface.
...
- **Shape:** Kubernetes manifests under `terraform/kubernetes/apps/` plus
  Gitea repo pushes for GitOps reconciliation.
...
- **Safe pre-launch changes:** publishing additional tags, adding repos to
  the Gitea org, adding new ingress aliases.
- **Breaking changes:** renaming the existing workload repos, renaming image
  namespaces, or changing ingress hostnames that runbooks assume.
