...
- `subnet-calculator/apim-simulator/` (config-driven gateway simulator)
...
`subnet-calculator/apim-simulator/` (stack12):
...
`subnet-calculator/apim-simulator/`:
...
- `subnet-calculator-a57` — APIM simulator: Docker-first, feature-complete authn/z + policy engine

Children:

- `subnet-calculator-a57.1` Capability matrix mapped to Terraform resources
- `subnet-calculator-a57.2` Pick canonical simulator and migration plan
- `subnet-calculator-a57.3` Extend config model (APIs/operations/products/subscriptions/groups/users/named values)
- `subnet-calculator-a57.4` Management plane endpoints + tenant access keys
- `subnet-calculator-a57.5` Policy engine v1 pipeline + choose/when
- `subnet-calculator-a57.6` Validate-jwt policy (multi-issuer, aud, claims/scopes/roles)
- `subnet-calculator-a57.7` Subscription key semantics + entitlement + states
- `subnet-calculator-a57.8` Rate-limit + quota policies
- `subnet-calculator-a57.9` Baseline inbound policies
- `subnet-calculator-a57.10` Backend definitions + backend auth simulation
- `subnet-calculator-a57.11` TLS/mTLS frontdoor support
- `subnet-calculator-a57.12` Tracing + diagnostics
- `subnet-calculator-a57.13` Terraform plan/state JSON importer
- `subnet-calculator-a57.14` Test harness
- `subnet-calculator-a57.15` Kubernetes deployment ergonomics
