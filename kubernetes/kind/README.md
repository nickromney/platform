...
- Stage `900` is the confidence path when you drive it through `make`. A successful `make kind apply 900` now runs `check-health`, `check-gateway-urls`, and `check-sso-e2e` before returning success on both the host and the devcontainer. Stage `800` also runs `check-health` and `check-gateway-urls` after apply so the shared HTTPS entrypoints have to be up, not just rendered. Raw Terragrunt/OpenTofu applies remain apply-only.
