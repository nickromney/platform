...
| variant adapter | the implementation behind a variant that satisfies platform contracts | May be Terraform/OpenTofu, Terragrunt, scripts, provider APIs, or a mix of them. The kind and lima Makefiles are execution adapters (ADR 0006); shared lifecycle lives in the variant lifecycle module, not copied between them. |
| variant lifecycle module | the shared Make targets both local variant adapters include | `mk/k8s-variant-lifecycle.mk`: `state-reset`, `check-kubeconfig`, `gitea-sync`, and the conflicting-cluster check. Adapters parameterise it (`KUBECONFIG_STACK_LABEL`, `CONFLICTING_CLUSTER_CHECK`, `GITEA_SYNC_*`) and keep only what genuinely differs. A body with more than three real differences stays in the adapter. |
| tfvar resolution order | the one ordered `--optional-file` list every plan/apply/check site passes | Defined once in `mk/k8s-terragrunt.mk` as `stack_tfvar_common` / `stack_tfvar_args` / `stack_tfvar_args_full`. Order is stage, target, profile, base, platform, operator overrides. `_full` always includes the stage file; the unsuffixed form omits it unless the operator named a stage. |
...
| workflow core | non-interactive command builder for guided workflows | Implemented by `scripts/platform-workflow.sh`; it previews or runs Makefile commands and writes generated tfvars only when toggles require them. It takes a positional **subcommand** (`options`, `preview`, `apply`, `save-profile`) and a separate `--action` flag; the two vocabularies overlap only on `apply`. |
| workflow core module | the shared Go library both guided surfaces build their argv with | `tools/platform-workflow-core`. Owns `Args`, `AppDefault`, `ActionUsesAutoApprove`, and the streaming `Run`. A surface that builds argv itself will drift from the other one; every divergence has been a bug. |
...
| operator facts | the resolved operator intent Terraform renders for shell consumers | `${run_dir}/operator-facts.json`, emitted by `terraform/kubernetes/operator-facts.tf`. Checkers `source` `operator-facts.sh` and read it through `operator_facts_get` / `_bool` / `_list` instead of regex-parsing `.tfvars`. `OPERATOR_FACTS_FILE` wins when present; the tfvars parse is the pre-apply fallback. Carries no secrets. |
| guided surface | interactive wrapper over the workflow core | Current guided surfaces are the Bubble Tea TUI and the Go/HTMX browser workflow UI. Both bind to the workflow core module rather than assembling argv themselves, and both must feed it the same `options.json` facts — including `ui_rules`, which duplicates `stages[].app_toggles`. |
