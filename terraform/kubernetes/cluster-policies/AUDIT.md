...
10. Reworked namespace metadata from a two-way `application/shared` split to a three-way `application/shared/platform` taxonomy, replaced the old `security-tier` wording with `platform.publiccloudexperiments.net/sensitivity`, and added an empty `sit` application namespace to prove namespace-level inheritance without deploying workloads there.
