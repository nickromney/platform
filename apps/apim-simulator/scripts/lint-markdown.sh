...
list_markdown_files() {
  if tool_exists "${GIT_BIN}" && "${GIT_BIN}" -C "${REPO_ROOT}" rev-parse --is-inside-work-tree >/dev/null 2>&1; then
    "${GIT_BIN}" -C "${REPO_ROOT}" ls-files -z -- \
      '*.md' \
      '*.markdown' \
      ':(exclude)docs/walkthrough*.md'
    return 0
  fi

  find "${REPO_ROOT}" \
    \( -path '*/.git' -o -path '*/node_modules' -o -path '*/.venv' -o -path '*/.terraform' -o -path '*/dist' -o -path "${REPO_ROOT}/docs/walkthrough*.md" \) -prune \
...
run_markdownlint() {
  ...
  case "$(basename "${bin}")" in
    markdownlint)
      ...
      ;;
    markdownlint-cli2)
      local tmpdir tmpconfig cli2_path
      local -a cli2_files
