...
```bash
# Build image
docker build --platform linux/amd64 -t subnetcalc-python-api:v1 .

# Run locally
docker run --platform linux/amd64 --rm -it --init -p 8080:8080 subnetcalc-python-api:v1
```
...
```bash
# Build image
podman build --platform linux/amd64 --tag subnetcalc-python-api:v1 .

# Run locally
podman run --platform linux/amd64 --rm -it --init -p 8080:8080 subnetcalc-python-api:v1
```
