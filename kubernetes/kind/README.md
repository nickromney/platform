...
- Stage `900` is the confidence path when you drive it through `make`. On the host, a successful `make kind apply 900` now runs `check-health`, `check-gateway-urls`, and `check-sso-e2e` before returning success. Stage `800` also runs `check-health` and `check-gateway-urls` after apply so the shared HTTPS entrypoints have to be up, not just rendered. In the devcontainer, the stage-900 apply path still stops before browser E2E; host-oriented HTTPS and browser verification remain the reference path. Raw Terragrunt/OpenTofu applies remain apply-only.
