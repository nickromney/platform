...
```bash
docker build -t subnetcalc-frontend .
```
...
```bash
podman build -t subnetcalc-frontend .
```
...
```bash
# From the subnetcalc directory (parent)
cd ..
docker compose up
```
...
Browser → Flask (/lookup endpoint)
          ...
          Flask → API (http://api:8080/api/v1/*)
          ↓
...
# Podman
podman logs -f subnetcalc_frontend_1
```
