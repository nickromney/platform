...
usage() {
  cat <<'EOF'
Usage: check-bash32-compat.sh [--dry-run] [--execute] [--path PATH]...

Scan tracked shell scripts for Bash 4+ constructs that are incompatible with
macOS's stock Bash 3.2.

Without arguments, the scan covers every tracked *.sh file in the repo. When
one or more --path values are supplied, only those files/directories are
...
while [[ $# -gt 0 ]]; do
  ...
  case "$1" in
    --path)
      shift
      [[ $# -gt 0 ]] || { shell_cli_missing_value "$(shell_cli_script_name)" "--path"; exit 1; }
      scan_paths+=("$1")
      ;;
    --)
      shift
