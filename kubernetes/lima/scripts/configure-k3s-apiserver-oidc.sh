...
remote_read_file() {
  local path="$1"
  lima_exec "$LIMA_NODE_NAME" sudo sh -c "cat '$path' 2>/dev/null || true"
}

remote_write_file() {
  local path="$1"
  local content="$2"
  lima_exec "$LIMA_NODE_NAME" sudo mkdir -p "$(dirname "$path")"
  printf '%s\n' "$content" | lima_exec "$LIMA_NODE_NAME" sudo tee "$path" >/dev/null
}
...
ensure_remote_host_alias() {
  ...
  lima_exec "$LIMA_NODE_NAME" sudo env DEX_HOST="$DEX_HOST" GATEWAY_IP="$desired_ip" bash -s <<'EOF'
set -euo pipefail
...
ensure_remote_ca() {
  ...
  remote_sha="$(lima_exec "$LIMA_NODE_NAME" sudo sh -c "sha256sum '$dest_path' 2>/dev/null | cut -d' ' -f1" || true)"

  if [[ "$local_sha" == "$remote_sha" ]]; then
    return 1
  fi

  lima_exec "$LIMA_NODE_NAME" sudo mkdir -p "$(dirname "$dest_path")"
  lima_exec "$LIMA_NODE_NAME" sudo tee "$dest_path" >/dev/null <"$source_ca"
  return 0
}
...
if ! command -v mkcert >/dev/null 2>&1; then
  warn "mkcert not found; skipping Lima apiserver OIDC configuration"
...
if [[ ! -f "$CA_CERT" ]]; then
  warn "mkcert CA cert not found at ${CA_CERT}; skipping Lima apiserver OIDC configuration"
  exit 0
fi
...
if [[ -n "$hosts_changed" || "$ca_changed" == "1" || "$config_changed" == "1" ]]; then
  ...
  lima_exec "$LIMA_NODE_NAME" sudo systemctl restart k3s
else
  ok "k3s OIDC settings already current"
fi

ok "waiting for OIDC issuer discovery endpoint from Lima VM"
end=$((SECONDS + OIDC_DISCOVERY_WAIT_SECONDS))
while (( SECONDS < end )); do
  if lima_exec "$LIMA_NODE_NAME" sudo curl -fsS --max-time 5 --cacert "$MKCERT_CA_DEST" "${OIDC_ISSUER_URL}/.well-known/openid-configuration" >/dev/null; then
    ok "OIDC issuer reachable from Lima VM: ${OIDC_ISSUER_URL}"
    break
  fi
  sleep 2
done
if (( SECONDS >= end )); then
  fail "timed out waiting for ${OIDC_ISSUER_URL}/.well-known/openid-configuration (from Lima VM)"
fi
