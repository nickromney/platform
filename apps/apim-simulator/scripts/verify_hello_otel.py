...
def main() -> None:
    ...
    verify_tls = resolve_tls_verify(
        ...
        ca_env="VERIFY_HELLO_OTEL_CA_CERT",
        verify_env="VERIFY_HELLO_OTEL_VERIFY_TLS",
        insecure_env="VERIFY_HELLO_OTEL_INSECURE_SKIP_VERIFY",
    )
    ...
    with httpx.Client(
        ...
        if not any(value.startswith("Hello API:") for value in apim_route_values):
            raise RuntimeError("Tempo does not include Hello API route tags")
