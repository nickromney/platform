...
| `sentiment-api-dns-egress.yaml` | Allows DNS for sentiment backend and LLM workloads in application namespaces. | Good narrow helper policy. |
...
| --- | --- | --- |
| `projects/sentiment/sentiment-runtime.yaml` | Reusable namespaced runtime policy set for sentiment router, API, frontend, LiteLLM, and llama.cpp. | Rendered as `CiliumNetworkPolicy` into `dev`, `uat`, and `sit`. |
