...
if kubectl -n argocd get applications.argoproj.io >/dev/null 2>&1; then
  ...
  expected_apps=(cert-manager cert-manager-config platform-gateway platform-gateway-routes kyverno kyverno-policies cilium-policies gitea gitea-actions-runner apim dev uat oauth2-proxy-argocd oauth2-proxy-gitea oauth2-proxy-grafana oauth2-proxy-hubble oauth2-proxy-sentiment-dev oauth2-proxy-sentiment-uat oauth2-proxy-subnetcalc-dev oauth2-proxy-subnetcalc-uat prometheus grafana victoria-logs headlamp)
  # nginx-gateway-fabric is a pre-Cilium-Gateway migration reference, not a
  # required app on the kind operator path.
  if ! kubectl get gatewayclass cilium >/dev/null 2>&1; then
    expected_apps+=(nginx-gateway-fabric)
  fi
