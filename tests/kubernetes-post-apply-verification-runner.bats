...
@test "post-apply runner preserves Slicer configure stage contract" {
  install_fake_make
  export MAKE_CAPTURE="${BATS_TEST_TMPDIR}/make-calls"

  run bash -c 'printf "%s\n" configure-k3s-apiserver-oidc check-health | "$SCRIPT" --execute --variant-json "$REPO_ROOT/kubernetes/variants/slicer/variant.json" --stage 900 --make-dir "$REPO_ROOT/kubernetes/slicer"'

  [ "${status}" -eq 0 ]
  expected=$'-C '"${REPO_ROOT}"$'/kubernetes/slicer configure-k3s-apiserver-oidc STAGE=900\n-C '"${REPO_ROOT}"$'/kubernetes/slicer check-health STAGE=900'
  [ "$(cat "${MAKE_CAPTURE}")" = "${expected}" ]
}
